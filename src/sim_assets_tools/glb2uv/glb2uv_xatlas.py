...
from lib.constants import COLOR_BLACK_VISOR, COLOR_WHITE_SHELL, DARK_THRESHOLD
from lib.mesh_classify import load_glb_with_face_colors
from lib.obj_io import parse_obj, write_obj
from lib.texture_utils import dilate_texture, rasterize_face_colors
...
def match_faces_by_centroid(glb_verts, glb_faces, obj_verts, obj_faces):
    ...
    print(f"    Face matching: max dist={dists.max():.6f}, " f"mean dist={dists.mean():.6f}")
    if dists.max() > 0.01:
...
def process_glb_with_existing_uv(glb_path, obj_path, tex_output, tex_size=2048, dilate_iters=24):
    ...
    print(f"  OBJ: {len(obj_verts)} verts, {len(obj_uvs)} UVs, " f"{len(obj_faces)} faces")
    ...
    t0 = time.time()
    glb_indices = match_faces_by_centroid(glb_verts_matched, glb_faces, obj_verts, obj_faces)
...
def process_glb(glb_path, obj_output, tex_output, tex_size=2048, dilate_iters=24):
    ...
    print(
        f"  UV done in {time.time() - t0:.1f}s: " f"{len(new_verts)} verts, {len(new_faces)} faces"
    )
...
def main():
    parser = argparse.ArgumentParser(description="GLB -> OBJ + PNG texture using xatlas")
        ...
        try:
            if args.existing_uv:
                ...
                if not os.path.exists(obj_path):
                    candidates = [
                        f
                        for f in os.listdir(args.existing_uv)
                        if f.endswith(".obj") and glb_name in f
                    ]
                    if candidates:
                        obj_path = os.path.join(args.existing_uv, candidates[0])
                    else:
                        print(f"  SKIP {glb_name}: no matching OBJ in " f"{args.existing_uv}")
                        results.append((out_name, False))
                        continue
                ok = process_glb_with_existing_uv(
                    glb_path, obj_path, tex_path, tex_size=args.tex_size, dilate_iters=args.dilate
                )
            else:
