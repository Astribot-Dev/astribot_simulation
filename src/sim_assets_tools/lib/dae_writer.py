...
def write_dae_flat(filepath, material_groups):
    ...
    for grp in material_groups:
        ...
        SubElement(
            diff_el, "color", {"sid": "diffuse"}
        ).text = f"{diffuse[0]:.7f} {diffuse[1]:.7f} {diffuse[2]:.7f} 1"

        mat = SubElement(
            lib_mats, "material", {"id": f"{mid}-material", "name": grp["material_name"]}
        )
    ...
    SubElement(
        pos_source, "float_array", {"id": "mesh0-positions-array", "count": str(n_verts * 3)}
    ...
    SubElement(
        norm_source, "float_array", {"id": "mesh0-normals-array", "count": str(n_verts * 3)}
    ...
    SubElement(vertices, "input", {"semantic": "POSITION", "source": "#mesh0-positions"})
    ...
    for i, grp in enumerate(material_groups):
        ...
        triangles = SubElement(
            mesh, "triangles", {"count": str(len(faces)), "material": f"{mid}-material"}
        )
        SubElement(
            triangles, "input", {"semantic": "VERTEX", "source": "#mesh0-vertices", "offset": "0"}
        )
        SubElement(
            triangles, "input", {"semantic": "NORMAL", "source": "#mesh0-normals", "offset": "1"}
        )
    ...
    vis_scene = SubElement(lib_scenes, "visual_scene", {"id": "Scene", "name": "Scene"})
    node = SubElement(vis_scene, "node", {"id": "node0", "name": "mesh0", "type": "NODE"})
...
def write_dae_vertex_color(filepath, positions, colors, normals, faces):
    ...
    norm_source = SubElement(mesh, "source", {"id": "mesh0-normals"})
    SubElement(
        norm_source, "float_array", {"id": "mesh0-normals-array", "count": str(n_normals * 3)}
    ).text = " ".join(f"{n:.8f}" for n in normals.flatten())
    norm_tech = SubElement(norm_source, "technique_common")
    norm_acc = SubElement(
        norm_tech,
    ...
    SubElement(
        col_source, "float_array", {"id": "mesh0-colors-array", "count": str(n_verts * 4)}
    ...
    # Triangles
    triangles = SubElement(mesh, "triangles", {"count": str(n_faces), "material": "material0"})
    SubElement(
        triangles, "input", {"semantic": "VERTEX", "source": "#mesh0-vertices", "offset": "0"}
    )
    SubElement(
    ...
    SubElement(triangles, "input", {"semantic": "COLOR", "source": "#mesh0-colors", "offset": "0"})
    ...
    bind_mat = SubElement(inst_geom, "bind_material")
    tech_common = SubElement(bind_mat, "technique_common")
    SubElement(tech_common, "instance_material", {"symbol": "material0", "target": "#material0"})
...
def write_dae_textured(filepath, flat_groups, tex_group, tex_filename):
    ...
    for grp in flat_groups:
        ...
        SubElement(df, "color", {"sid": "diffuse"}).text = f"{d[0]:.7f} {d[1]:.7f} {d[2]:.7f} 1"

        mat = SubElement(lib_mats, "material", {"id": f"{mid}-material", "name": mid})
        SubElement(mat, "instance_effect", {"url": f"#{mid}-effect"})
    ...
    mat = SubElement(lib_mats, "material", {"id": f"{tmid}-material", "name": tmid})
    ...
    SubElement(src_p, "float_array", {"id": "mesh0-pos-arr", "count": str(nv * 3)}).text = " ".join(
        f"{x:.8f}" for x in positions.flatten()
    )
    tc = SubElement(src_p, "technique_common")
    acc = SubElement(tc, "accessor", {"source": "#mesh0-pos-arr", "count": str(nv), "stride": "3"})
    for p in "XYZ":
    ...
    SubElement(
        src_n, "float_array", {"id": "mesh0-norm-arr", "count": str(nv * 3)}
    ).text = " ".join(f"{x:.8f}" for x in normals_all.flatten())
    tc = SubElement(src_n, "technique_common")
    acc = SubElement(tc, "accessor", {"source": "#mesh0-norm-arr", "count": str(nv), "stride": "3"})
    for p in "XYZ":
        SubElement(acc, "param", {"name": p, "type": "float"})
    ...
    SubElement(
        src_uv, "float_array", {"id": "mesh0-uv-arr", "count": str(n_uv * 2)}
    ).text = " ".join(f"{x:.8f}" for x in uvs.flatten())
    tc = SubElement(src_uv, "technique_common")
    acc = SubElement(tc, "accessor", {"source": "#mesh0-uv-arr", "count": str(n_uv), "stride": "2"})
    ...
    SubElement(verts_el, "input", {"semantic": "POSITION", "source": "#mesh0-pos"})
    ...
    for i, g in enumerate(flat_groups):
        ...
        tri = SubElement(
            mesh_el, "triangles", {"count": str(len(faces)), "material": f"{mid}-material"}
        )
        SubElement(tri, "input", {"semantic": "VERTEX", "source": "#mesh0-verts", "offset": "0"})
        SubElement(tri, "input", {"semantic": "NORMAL", "source": "#mesh0-norm", "offset": "1"})
        pd = []
        for f in faces:
            for vi in f:
                pd.append(str(vi))
    ...
    tri = SubElement(
        mesh_el, "triangles", {"count": str(len(tex_faces)), "material": f"{tmid}-material"}
    )
    SubElement(tri, "input", {"semantic": "VERTEX", "source": "#mesh0-verts", "offset": "0"})
    SubElement(tri, "input", {"semantic": "NORMAL", "source": "#mesh0-norm", "offset": "1"})
    SubElement(
        tri, "input", {"semantic": "TEXCOORD", "source": "#mesh0-uv", "offset": "2", "set": "0"}
    )
    pd = []
    for f in tex_faces:
        for vi in f:
    ...
    nd = SubElement(vs, "node", {"id": "node0", "name": "mesh0", "type": "NODE"})
    ...
    for g in all_groups:
        mid = g["material_id"]
        SubElement(
            tc2, "instance_material", {"symbol": f"{mid}-material", "target": f"#{mid}-material"}
        )
