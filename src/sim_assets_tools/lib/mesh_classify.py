...
def load_glb_classified(glb_path, dark_threshold=DARK_THRESHOLD):
    ...
    if not isinstance(scene, trimesh.Scene):
        ...
        print(f"  Single mesh: {len(scene.vertices)} verts, " f"{len(scene.faces)} faces -> white")
    ...
    for name, geom in scene.geometry.items():
        ...
        print(
            f"  {name}: {len(geom.vertices)} verts, {len(geom.faces)} faces, "
...
def load_glb_with_face_colors(glb_path, white_color, black_color, dark_threshold=DARK_THRESHOLD):
    ...
    for name, geom in geometries:
        ...
        print(
            f"    {name}: {len(geom.vertices)} verts, {n_faces} faces, "
