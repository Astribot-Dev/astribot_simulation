...
class AstribotAgent(BaseAgent):
    ...
    def _sensor_configs(self):
        return [
            CameraConfig(
                uid="astribot_arm_left_effector",
                pose=sapien.Pose(p=[0, 0.1, 0], q=[0.5,-0.5,-0.5,-0.5]),
                width=640,
                height=480,
                fov=(np.pi / 3)*2,
                near=0.01,
            ...
            CameraConfig(
                uid="astribot_arm_right_effector",
                pose=sapien.Pose(p=[0, 0.1, 0], q=[0.5,-0.5,-0.5,-0.5]),
                width=640,
                height=480,
                fov=(np.pi / 3)*2,
            ...
            CameraConfig(
                uid="astribot_head",
                pose=sapien.Pose(p=[0.1, 0, 0.13], q=[0, 1, 0, 0]),
                width=3840,
                height=2160,
                fov=(np.pi / 3) * 4,
                near=0.01,
                far=100,
                mount=self.robot.links_map["astribot_head_base_link"],
            )
        ]
...
class AstribotManiskillEnv(BaseEnv, AstribotBaseEnv):
    ...
    def get_gravity_torque(self):
        zero_vector = np.zeros(self.agent.robot.get_dof(), dtype=np.float32)
