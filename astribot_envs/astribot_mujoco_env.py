...
class AstribotMujocoEnv(AstribotBaseEnv):
    ...
    def close(self):
        if self.render_mode=="human":
            ...
            for camera_name in self.camera_names:
                self.camera_dict[camera_name].close()
    ...
    def get_body_pose(self, body_name):
        if not self.check_body(body_name):
            astribot_simu_log("Do not have this body: ", body_name, "ERROR")
    ...
    def get_camera_image(self, camera_name='top'):
        ...
        point_cloud=self.trans_depth_image_to_point_cloud(depth_img, self.height, self.width, camera_name)
