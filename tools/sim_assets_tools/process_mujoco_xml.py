...
def process_single_object(path, object_name):
    ...
    asset_xml = asset_xml.replace(
        'file="',
        'file="worldbody/mujoco_scanned_objects/models/' + object_name + "/")
    asset_xml = asset_xml.replace('texture="', 'texture="' + object_name + "_texture")
    asset_xml = asset_xml.replace(
        'texture="' + object_name + "_texturetexture",
        'texture="' + object_name + "_texture")
    ...
    worldbody_xml = worldbody_xml.replace(
        '<geom',
        '<geom friction="0.9" solimp="0.95 0.99 0.001" solref="0.001 1" priority="1"')
...
def generate_aggregate_xml(path, object_index_start=9, object_index_end=15):
    ...
    directory_list = [d for d in os.listdir(path)
    ...
    with open(asset_path, 'w') as f:
        f.write('<mujocoinclude>\n')
        for name in directory_list:
            f.write(f'<include file="worldbody/mujoco_scanned_objects/models/'
...
def main():
    parser = argparse.ArgumentParser(
        description="Process MuJoCo scanned object XML files")
    parser.add_argument("--path", default=None,
                        help="Path to scanned objects directory")
    parser.add_argument("--start", type=int, default=9,
                        help="Object index start (default: 9)")
    parser.add_argument("--end", type=int, default=15,
                        help="Object index end (default: 15)")
    args = parser.parse_args()

    path = args.path or ("/home/robot/astribot_ws/src/astribot_robotics/"
